module.exports = function share(
  ...
  (function () {
      ...
      Sms.sendMeText.skip();
      Buffer.addPostToQueue2.setPostText(tweet());
      shareToThreads(thread());
      if (isProfessional(input.tags)) shareToLinkedIn();
      else Buffer.addPostToQueue1.skip();
      ...
      function shareToLinkedIn() {
        Buffer.addPostToQueue1.setPostText(toot());
      }
      ...
      function shareToThreads(content: string) {
        Buffer.addPostToQueue3.setPostText(content)
      }
      ...
      function debug() {
        Buffer.addPostToQueue1.skip();
        Buffer.addPostToQueue2.skip();
        Buffer.addPostToQueue3.skip();
