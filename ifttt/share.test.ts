...
describe("The share filter", () => {
  ...
  describe('when debugging', () => {
    ...
    it('should SMS the Tweet', function () {
      expect(Sms.sendMeText.setMessage).toHaveBeenCalledWith(`Tweet: "Whatever you're working on right now, whatever it might be, I ask: try to leave a little space for a courtyard."\n\n${url} #test`)
    });

    it('should SMS the Toot', function () {
      expect(Sms.sendMeText.setMessage).toHaveBeenCalledWith(`Toot: "Whatever you're working on right now, whatever it might be, I ask: try to leave a little space for a courtyard."\n\n${url} #test`)
    });

    it('should SMS the LinkedIn comment', function () {
      expect(Sms.sendMeText.setMessage).toHaveBeenCalledWith(`LinkedIn: "Whatever you're working on right now, whatever it might be, I ask: try to leave a little space for a courtyard."\n\n#test`)
    });
...
function post(body: string, url: string, tags: string) {
  share(
      ...
      Twitter, Linkedin, MakerWebhooks, Sms,
      "hachyderm.io",
      "t00ts");
}
